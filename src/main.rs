use i1::*;
use metro;
use reedline_repl_rs::clap::{self, Arg, Command, builder::TypedValueParser};
...
impl TypedValueParser for CommitUuidParser {
    ...
    fn parse_ref(&self, cmd: &Command, arg: Option<&Arg>, value: &std::ffi::OsStr) -> Result<Self::Value, clap::Error> {
        let inner = reedline_repl_rs::clap::builder::StringValueParser::default().parse(cmd, arg, value.into())?;
        let uuid = Uuid::from_base64url(&inner).map_err(|_| clap::Error::raw(clap::error::ErrorKind::InvalidValue, format!("Invalid UUID: {}", inner)))?;
        let state = self.state.read().unwrap();
        if state.universe.get_index_of(&uuid).map(|luid| state.commits.contains_key(&luid)).unwrap_or(false) {
            Ok(uuid)
        } else {
            Err(clap::Error::raw(clap::error::ErrorKind::InvalidValue, format!("Unknown commit: {}", inner)))
        }
    }
    fn possible_values( &self,) -> Option<Box<dyn Iterator<Item = clap::builder::PossibleValue> + '_>> {
        ...
        Some(Box::new(commits.into_iter().map(|s| clap::builder::PossibleValue::new(s))))
    }
} 
    ...
    let mut repl: Repl<_, Error> = Repl::new(state.clone())
        ...
        .with_command(
            ...
            |_,state| {
                ...
                Ok(Some(format!("Created new entity {} in working set", new_uuid.as_base64url())))
            })
        .with_command(
        ...
        .with_command(
            ...
            |_,state| {
                ...
                Ok(Some(format!("Saved new patch {} from [{}] to {}",
                    patch_id.as_base64url(),
                    patch.source_commits.iter().map(|id| id.as_base64url()).collect::<Vec<_>>().join(", "),
            ...
            |_,state| {
                ...
                Ok(Some(
                    metro::to_string(
                        graph
                          .iter().map(|e| e.into())
                          .collect::<Vec<metro::Event>>().as_slice())
